...
const _sfc_main = common_vendor.defineComponent({
  ...
  onShow() {
    common_vendor.index.__f__("log", "at pages/index/my.uvue:180", "页面显示，检查连接状态");
    ...
    if (!savedDeviceId) {
      ...
      common_vendor.index.__f__("log", "at pages/index/my.uvue:189", "已有连接设备，不重新搜索");
      this.checkConnectedDevice();
    }
  },
  onUnload() {
    common_vendor.index.__f__("log", "at pages/index/my.uvue:195", "页面卸载");
  ...
  methods: {
    // 初始化数据
    initData() {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:205", "我的页面数据初始化");
    },
    // 初始化蓝牙适配器
    initBluetooth() {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:212", "初始化蓝牙适配器");
      ...
      common_vendor.index.getBluetoothAdapterState(new UTSJSONObject({
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:237", "getBluetoothAdapterState", res);
          if (!res.available) {
            that.deviceStatus = "蓝牙不可用";
            that.bluetoothEnabled = false;
          } else {
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:254", "获取蓝牙适配器状态失败", err);
          that.deviceStatus = "蓝牙不可用";
          that.bluetoothEnabled = false;
        }
      }));
    ...
    openBluetooth() {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:280", "打开蓝牙");
      common_vendor.index.__f__("log", "at pages/index/my.uvue:281", common_vendor.index.getSystemInfoSync().platform);
      if (common_vendor.index.getSystemInfoSync().platform === "android") {
        common_vendor.index.openBluetoothAdapter(new UTSJSONObject({
          success: (res = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:286", "openBluetoothAdapter success", res);
            that.bluetoothEnabled = true;
          ...
          fail: (res = null) => {
            if (res.errCode === 10001) {
              common_vendor.index.onBluetoothAdapterStateChange((res2 = null) => {
                common_vendor.index.__f__("log", "at pages/index/my.uvue:295", "onBluetoothAdapterStateChange", res2);
                if (res2.available) {
                  that.bluetoothEnabled = true;
                  that.deviceStatus = "蓝牙已开启";
                  that.searchDevices();
        ...
        common_vendor.index.openBluetoothAdapter(new UTSJSONObject({
          success: (res = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:311", "openBluetoothAdapter success", res);
            that.bluetoothEnabled = true;
            that.deviceStatus = "蓝牙已开启";
            that.searchDevices();
          },
    ...
    closeBluetooth() {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:328", "关闭蓝牙");
      common_vendor.index.closeBluetoothAdapter(new UTSJSONObject({
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:331", "关闭蓝牙适配器成功", res);
          that.bluetoothEnabled = false;
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:340", "关闭蓝牙适配器失败", err);
        }
      }));
    },
    // 判断数组中是否包含某个元素
    ...
    searchDevices() {
      ...
      if (that.isSearching) {
        common_vendor.index.__f__("log", "at pages/index/my.uvue:369", "正在搜索中，请稍候...");
        return null;
      }
      common_vendor.index.__f__("log", "at pages/index/my.uvue:373", "开始搜索蓝牙设备");
      ...
      common_vendor.index.openBluetoothAdapter(new UTSJSONObject({
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:396", "蓝牙适配器已打开", res);
          common_vendor.index.startBluetoothDevicesDiscovery(new UTSJSONObject({
            ...
            success: (res2 = null) => {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:403", "开始搜索设备", res2);
              common_vendor.index.onBluetoothDeviceFound((res3 = null) => {
                ...
                res3.devices.forEach((device = null) => {
                  ...
                  if (!that.lastLogTime[deviceId] || now - that.lastLogTime[deviceId] >= that.logInterval) {
                    ...
                    common_vendor.index.__f__("log", "at pages/index/my.uvue:427", "发现设备:", device.name, "ID:", deviceId);
                  }
                  ...
                  if (idx === -1) {
                    if (device.name && device.name.includes("RideHDD")) {
                      common_vendor.index.__f__("log", "at pages/index/my.uvue:437", "添加新设备:", device.name);
                      ...
                      common_vendor.index.__f__("log", "at pages/index/my.uvue:448", "尝试连接设备:", device.name);
                    ...
                    if (!that.discoveredDevices[idx].connected && !that.connectedDeviceId) {
                      if (!that.lastConnectAttempt || now - that.lastConnectAttempt >= that.logInterval) {
                        ...
                        common_vendor.index.__f__("log", "at pages/index/my.uvue:472", "尝试重新连接设备:", device.name);
            ...
            fail: (err = null) => {
              common_vendor.index.__f__("error", "at pages/index/my.uvue:481", "搜索设备失败", err);
          ...
          that.searchTimer = setInterval(() => {
            if (that.isSearching && !that.connectedDeviceId) {
              common_vendor.index.getBluetoothAdapterState(new UTSJSONObject({
                  ...
                  if (res2.available) {
                    if (!res2.discovering) {
                      common_vendor.index.startBluetoothDevicesDiscovery(new UTSJSONObject({
                        ...
                        fail: (err = null) => {
                          common_vendor.index.__f__("error", "at pages/index/my.uvue:505", "搜索设备失败", err);
                        }
                      }));
                    }
                  }
                ...
                fail: (err = null) => {
                  common_vendor.index.__f__("error", "at pages/index/my.uvue:512", "获取蓝牙适配器状态失败", err);
                  that.isSearching = false;
                  if (that.searchTimer) {
                    clearInterval(that.searchTimer);
                    that.searchTimer = null;
              ...
              common_vendor.index.__f__("log", "at pages/index/my.uvue:523", "已连接设备，停止搜索");
              that.stopSearchDevices();
            }
          }, 2e3);
          that.searchTimeoutTimer = setTimeout(() => {
            if (that.isSearching && !that.connectedDeviceId) {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:531", "搜索超时，自动停止搜索");
              that.stopSearchDevices();
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:538", "打开蓝牙适配器失败", err);
          that.isSearching = false;
          that.deviceStatus = "蓝牙开启失败";
          if (that.searchTimer) {
            clearInterval(that.searchTimer);
    ...
    stopSearchDevices() {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:554", "停止搜索蓝牙设备");
      if (that.searchTimer) {
        clearInterval(that.searchTimer);
        that.searchTimer = null;
      }
      ...
      common_vendor.index.getBluetoothAdapterState(new UTSJSONObject({
        success: (res = null) => {
          if (res.available && res.discovering) {
            common_vendor.index.stopBluetoothDevicesDiscovery(new UTSJSONObject({
              success: (res2 = null) => {
                common_vendor.index.__f__("log", "at pages/index/my.uvue:577", "停止搜索设备成功", res2);
              },
              fail: (err = null) => {
                common_vendor.index.__f__("error", "at pages/index/my.uvue:580", "停止搜索设备失败", err);
              }
            }));
          }
          that.isSearching = false;
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:593", "获取蓝牙适配器状态失败", err);
          that.isSearching = false;
        }
      }));
    },
    ...
    checkConnectedDevice() {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:602", "检查设备连接状态");
      const savedDeviceId = common_vendor.index.getStorageSync("connectedDeviceId");
      ...
      if (savedDeviceId) {
        ...
        common_vendor.index.openBluetoothAdapter(new UTSJSONObject({
          success: () => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:615", "蓝牙适配器已开启");
            this.checkDeviceConnection(savedDeviceId, savedDeviceName);
            this.bluetoothEnabled = true;
          },
          fail: (err = null) => {
            common_vendor.index.__f__("error", "at pages/index/my.uvue:621", "蓝牙适配器未开启，尝试初始化", err);
            this.initBluetooth();
        ...
        common_vendor.index.__f__("log", "at pages/index/my.uvue:632", "没有保存的连接设备");
        this.deviceStatus = "未连接";
      }
    },
    // 检查设备连接状态的核心方法
    checkDeviceConnection(savedDeviceId = null, savedDeviceName = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:639", "检查设备连接状态 - 核心方法", savedDeviceId);
      common_vendor.index.getConnectedBluetoothDevices(new UTSJSONObject({
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:644", "获取已连接设备列表", res);
          const isStillConnected = res.devices.some((device = null) => {
            return device.deviceId === savedDeviceId;
          });
          if (isStillConnected) {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:648", "设备仍然保持连接");
            this.updateConnectedState(savedDeviceId, savedDeviceName);
          } else {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:651", "设备连接已断开");
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:659", "获取已连接设备列表失败", err);
          this.updateConnectedState(savedDeviceId, savedDeviceName);
          setTimeout(() => {
    ...
    updateConnectedState(savedDeviceId = null, savedDeviceName = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:673", "更新已连接状态", savedDeviceId);
    ...
    connectDevice(device = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:703", "连接设备", device);
      if (!that.bluetoothEnabled) {
        common_vendor.index.showToast({
          title: "请先开启蓝牙",
          icon: "none"
      ...
      common_vendor.index.createBLEConnection(new UTSJSONObject({
        ...
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:725", "连接设备成功", res);
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:752", "连接设备失败", err);
    ...
    verifyConnectionStatus() {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:783", "验证设备连接状态");
      if (!this.connectedDeviceId)
        return null;
      common_vendor.index.getBLEDeviceServices(new UTSJSONObject({
        deviceId: this.connectedDeviceId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:790", "连接状态验证成功，设备仍然在线");
        },
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:794", "连接状态验证失败，设备可能已断开", err);
    ...
    handleUnexpectedDisconnect() {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:803", "处理意外断开连接");
      this.stopConnectionMonitor();
    ...
    connectDevice(device = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:842", "连接设备", device);
      if (!that.bluetoothEnabled) {
        common_vendor.index.showToast({
          title: "请先开启蓝牙",
          icon: "none"
      ...
      common_vendor.index.createBLEConnection(new UTSJSONObject({
        deviceId: device.deviceId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:864", "连接设备成功", res);
          device.connected = true;
          that.connectedDeviceId = device.deviceId;
          ...
          that.startConnectionMonitor();
          that.$forceUpdate();
          common_vendor.index.__f__("log", "at pages/index/my.uvue:897", "deviceInfo当前状态:", UTS.JSON.stringify(that.deviceInfo));
          common_vendor.index.showToast({
            title: "连接成功",
            icon: "success"
          });
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:905", "连接设备失败", err);
          that.deviceStatus = "连接失败";
    ...
    disconnectDevice(device = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:922", "断开设备连接", device);
      const that = this;
      common_vendor.index.closeBLEConnection(new UTSJSONObject({
        deviceId: device.deviceId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:929", "断开设备连接成功", res);
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:964", "断开设备连接失败", err);
          common_vendor.index.showToast({
            title: "断开失败",
            icon: "none"
          });
    ...
    disconnectAllDevices() {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:976", "断开所有设备连接");
      const that = this;
    ...
    removeDevice(device = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:989", "移除设备", device);
      const that = this;
      if (device.connected) {
        that.disconnectDevice(device);
      }
    ...
    getBLEDeviceServices(deviceId = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1018", "获取设备服务", deviceId);
      const that = this;
      ...
      common_vendor.index.getBLEDeviceServices(new UTSJSONObject({
        deviceId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1028", "获取设备服务成功", res);
          res.services.forEach((service = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1032", "服务UUID:", service.uuid);
          ...
          if (that.bleServices["0x180a"] || that.bleServices["180a"]) {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1042", "找到设备信息服务，尝试读取设备信息");
            setTimeout(() => {
              that.readDeviceInformation(deviceId);
            }, 500);
          }
          if (that.bleServices["0x180f"] || that.bleServices["180f"]) {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1051", "找到电池服务，尝试读取电池电量");
            setTimeout(() => {
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:1059", "获取设备服务失败", err);
        }
      }));
    },
    // 读取设备信息
    readDeviceInformation(deviceId = null) {
      var e_1, _a;
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1067", "尝试读取设备信息", deviceId);
      ...
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1081", "已发现的服务列表:", serviceKeys);
      try {
        for (var serviceKeys_1 = common_vendor.__values(serviceKeys), serviceKeys_1_1 = serviceKeys_1.next(); !serviceKeys_1_1.done; serviceKeys_1_1 = serviceKeys_1.next()) {
          ...
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1085", `检查服务: ${key} (normalized: ${normalizedKey})`);
          if (normalizedKey === "180a") {
            serviceId = key;
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1088", "找到设备信息服务:", serviceId);
            break;
          }
        }
      } catch (e_1_1) {
      ...
      if (serviceId && that.bleCharacteristics[serviceId]) {
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1094", "设备信息服务的特征值列表:", Object.keys(that.bleCharacteristics[serviceId]));
        ...
        const readNextCharacteristic = () => {
          ...
          if (readIndex >= characteristicsToRead.length) {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1108", "设备信息读取完成");
            return null;
          }
          ...
          try {
            for (var charKeys_1 = common_vendor.__values(charKeys), charKeys_1_1 = charKeys_1.next(); !charKeys_1_1.done; charKeys_1_1 = charKeys_1.next()) {
              ...
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1119", `检查特征值: ${charKey} (normalized: ${normalizedCharKey})，寻找: ${id}`);
              if (normalizedCharKey === id) {
                charId = charKey;
                common_vendor.index.__f__("log", "at pages/index/my.uvue:1122", `找到${name}特征值: ${charId}`);
                break;
              }
            }
          } catch (e_2_1) {
          ...
          if (charId) {
            ...
            if (char.properties.read) {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1132", `尝试读取${name}: ${serviceId}/${charId}`);
              setTimeout(() => {
              ...
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1140", `${name}特征值不支持读取`);
              if (char.properties.notify || char.properties.indicate) {
                common_vendor.index.__f__("log", "at pages/index/my.uvue:1143", `尝试启用${name}特征值通知`);
                setTimeout(() => {
            ...
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1154", `${name}特征值未找到 (寻找标准化的: ${id})`);
            readIndex++;
            readNextCharacteristic();
          }
        };
        ...
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1163", "未找到设备信息服务或特征值");
    ...
    readBatteryLevel(deviceId = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1172", "尝试读取电池电量", deviceId);
      ...
      if (serviceId && that.bleCharacteristics[serviceId] && (that.bleCharacteristics[serviceId][characteristicId] || that.bleCharacteristics[serviceId][characteristicId.replace("0x", "")])) {
        ...
        const char = that.bleCharacteristics[serviceId][charId];
        if (char.properties.read) {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1185", "尝试读取电池电量");
          that.readDeviceCharacteristicValue(deviceId, serviceId, charId);
        } else if (char.properties.notify) {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1188", "电池电量特征值支持通知，设置通知");
          that.notifyBLECharacteristicValueChange(deviceId, serviceId, charId);
        }
      }
    },
    ...
    getBLEDeviceCharacteristics(deviceId = null, serviceId = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1198", "获取设备特征值", deviceId, serviceId);
      ...
      common_vendor.index.getBLEDeviceCharacteristics(new UTSJSONObject({
        ...
        serviceId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1214", "获取设备特征值成功", res);
          res.characteristics.forEach((characteristic = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1218", "特征值UUID:", characteristic.uuid);
        ...
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:1238", "获取设备特征值失败", err);
        }
      }));
    },
    // 启用蓝牙设备特征值变化通知
    notifyBLECharacteristicValueChange(deviceId = null, serviceId = null, characteristicId = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1247", "启用蓝牙设备特征值变化通知", deviceId, serviceId, characteristicId);
      common_vendor.index.offBLECharacteristicValueChange();
      common_vendor.index.onBLECharacteristicValueChange((res = null) => {
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1254", "特征值变化", res);
      ...
      common_vendor.index.notifyBLECharacteristicValueChange(new UTSJSONObject({
        ...
        state: true,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1269", "启用蓝牙设备特征值变化通知成功", res);
        },
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:1272", "启用蓝牙设备特征值变化通知失败", err);
        }
      }));
    },
    // 处理接收到的数据
    handleReceivedData(buffer = null) {
      ...
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1281", "接收到的数据:", str);
    },
    // ArrayBuffer转16进制字符串
    ab2hex(buffer = null) {
    ...
    readDeviceCharacteristicValue(deviceId = null, serviceId = null, characteristicId = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1303", "读取设备特征值数据", deviceId, serviceId, characteristicId);
      common_vendor.index.readBLECharacteristicValue(new UTSJSONObject({
        deviceId,
        serviceId,
        characteristicId,
        success: (res = null) => {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1310", "读取特征值成功", res);
        },
        fail: (err = null) => {
          common_vendor.index.__f__("error", "at pages/index/my.uvue:1314", "读取特征值失败", err);
        }
      }));
    },
    // 监听特征值变化
    setupCharacteristicListener() {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1323", "设置特征值监听器");
      common_vendor.index.offBLECharacteristicValueChange();
      ...
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1330", "特征值监听器设置完成");
    },
    // 直接读取设备信息（备用方法）
    directReadDeviceInfo(deviceId = null) {
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1337", "尝试直接读取设备信息");
      ...
      const tryReadCharacteristic = () => {
        if (serviceIndex >= serviceIds.length) {
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1364", "直接读取设备信息完成");
          return null;
        }
        ...
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1392", `尝试直接读取 ${name}: ${serviceId}/${characteristicId}`);
        common_vendor.index.readBLECharacteristicValue(new UTSJSONObject({
          deviceId,
          serviceId,
          characteristicId,
          success: (res = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1399", `直接读取${name}成功，等待数据回调`, res);
            charIndex++;
            charIdIndex = 0;
            setTimeout(tryReadCharacteristic, 300);
          },
          fail: (err = null) => {
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1406", `直接读取${name}失败 (${serviceId}/${characteristicId}):`, err);
            charIdIndex++;
    ...
    handleCharacteristicValueChange(res = null) {
      const that = this;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1422", "特征值变化", res);
      ...
      if (deviceId !== that.connectedDeviceId) {
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1427", "忽略非当前连接设备的数据");
        return null;
      }
      ...
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1448", `Service: ${serviceId} (normalized: ${normalizedServiceId}), Characteristic: ${characteristicId} (normalized: ${normalizedCharId}), Value: ${hexString}, String: "${strValue}"`);
      if (!that.deviceInfo.show) {
        that.deviceInfo.show = true;
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1453", "设置deviceInfo.show为true");
      }
      if (normalizedServiceId === "180f") {
        if (normalizedCharId === "2a19") {
          try {
            ...
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1463", "电池电量:", that.deviceInfo.batteryLevel);
          } catch (error) {
            common_vendor.index.__f__("error", "at pages/index/my.uvue:1465", "解析电池电量失败:", error);
          }
        }
      }
      if (normalizedServiceId === "180a") {
        common_vendor.index.__f__("log", "at pages/index/my.uvue:1472", "检测到设备信息服务(180a)，处理特征值:", normalizedCharId);
        switch (normalizedCharId) {
          case "2a24":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1475", "检测到设备型号特征值(2a24)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.model = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1478", "更新设备型号为:", that.deviceInfo.model);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1480", "设备型号值为空或只有空白字符");
            }
            break;
          case "2a25":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1484", "检测到序列号特征值(2a25)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.serialNumber = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1487", "更新序列号为:", that.deviceInfo.serialNumber);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1489", "序列号值为空或只有空白字符");
            }
            break;
          case "2a26":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1493", "检测到固件版本特征值(2a26)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.firmwareVersion = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1496", "更新固件版本为:", that.deviceInfo.firmwareVersion);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1498", "固件版本值为空或只有空白字符");
            }
            break;
          case "2a27":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1502", "检测到硬件版本特征值(2a27)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.hardwareVersion = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1505", "更新硬件版本为:", that.deviceInfo.hardwareVersion);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1507", "硬件版本值为空或只有空白字符");
            }
            break;
          case "2a28":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1511", "检测到软件版本特征值(2a28)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.softwareVersion = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1514", "更新软件版本为:", that.deviceInfo.softwareVersion);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1516", "软件版本值为空或只有空白字符");
            }
            break;
          case "2a29":
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1520", "检测到制造商特征值(2a29)，原始值:", strValue);
            if (strValue && strValue.trim()) {
              that.deviceInfo.manufacturer = strValue.trim();
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1523", "更新制造商为:", that.deviceInfo.manufacturer);
            } else {
              common_vendor.index.__f__("log", "at pages/index/my.uvue:1525", "制造商值为空或只有空白字符");
            }
            break;
          default:
            common_vendor.index.__f__("log", "at pages/index/my.uvue:1529", "未识别的设备信息特征值:", normalizedCharId);
        }
      }
      if (that.deviceName === "智能骑行传感器 A1" && strValue && strValue.trim()) {
        if (normalizedCharId === "2a24" || normalizedCharId === "2a29") {
          ...
          common_vendor.index.__f__("log", "at pages/index/my.uvue:1538", "更新设备名称为:", that.deviceName);
        }
      }
      that.$forceUpdate();
    },
    ...
    ab2str(buffer = null) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/my.uvue:1558", "字符串转换失败:", error);
        return "";
      }
    },
    // 切换标签页
    switchTab(tab = null) {
      this.activeTab = tab;
      common_vendor.index.__f__("log", "at pages/index/my.uvue:1567", "切换到标签页:", tab);
      if (tab === "riding") {
